...
\section{The fundamental theorem of coalgebras}
...
\begin{proof}\
    \begin{enumerate}
            ...
            \[
            \Delta(c_i)=\sum\alpha_{ijk}c_j\otimes c_k
            \]
...
\begin{exercise}
  Prove that the set of group-like elements of a Hopf algebra is a group with
  the operation inherited from the algebra structure. 
\end{exercise}

% exercise 
% bialgebra homomorphism => Hopf algebra homomorphism
% Sweedler 4.0.1
...
with $\langle f\leftharpoonup h,x\rangle=\langle f,hx\rangle$.
...
\[
\sum (h\rightarrow c)_{1}\otimes(h\rightarrow c)_{2} =\sum (h_{1}\rightarrow c_{1})\otimes (h_{2}\rightarrow c_{2})
\quad\text{and}\quad 
...
\[
\sum (c\leftarrow h)_{1}\otimes(c\leftarrow h)_{2} =\sum (c_{1}\leftarrow h_{1})\otimes (c_{2}\leftarrow h_{2})
\quad\text{and}\quad 
...
\begin{align*}
\sum (1_A)_{-1}\otimes (1_A)_0 =1_{H}\otimes1_{A},
\quad\text{and}\quad 
