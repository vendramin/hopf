...
\begin{definition}
  ...
  \begin{equation}
    \label{eq:HopfModule}
    \delta(m\leftarrow h)=\sum (m_0\leftarrow h_1)\otimes m_1h_2
  \end{equation}
...
\[
  (m\otimes x)\leftarrow h=\sum (m\leftarrow h_1)\otimes xh_2
\]
...
\begin{example}[trivial Hopf modules]
  ...
  \[
    ...
    \delta(v\otimes h)=\sum (v\otimes h_1)\otimes h_2
  \] 
...
\begin{lemma}[Larson--Sweedler]
  ...
  module with 
  \[
    f\leftarrow h=\sum f_2(S(h))f_1,\quad 
  \delta(f)=\sum_{i=1}^n\sum f(h_{i,2})f_i\otimes h_{i,1}
  \]
...
\begin{proof}
  ...
  hand, 
  \[
  \delta(\varphi\leftarrow h) = \sum f_2(S(h))f_1 = 
  \sum_{i=1}^n\sum\varphi_2(S(h))f_i\otimes h_{i,1}\varphi_1(h_{i,2}).
  \]
  On the other hand, 
  \begin{align*}
    \sum (\varphi_0\leftarrow h_1)\otimes \varphi_1h_2 
    &=\sum_{i=1}^n\sum\varphi(h_{i,2})(f_i\leftarrow h_1)\otimes h_{i,1}h_2\\
    &=\sum_{i=1}^n\sum\varphi(h_{i,2})(f_{i,2}(S(h_1))f_{i,1}\otimes h_{i,1}h_2.
  \end{align*}
...
\begin{proposition}
  ...
  then $\int_{H^*}^l = (H^*)^{\operatorname{op}H}$. 
\end{proposition}
...
\begin{proof}
  ...
  $H^*\simeq (H^*)^{\operatorname{op}H}\otimes H$ as Hopf modules. Then 
  \[
    \dim H=\dim H^*=(\dim (H^*)^{\operatorname{op}H})(\dim H).
  \]
  and hence $\dim (H^*)^{\operatorname{op}H}=1$. Now 
...
\begin{proof}
  ...
  $S(x)=0$ implies that
  \[
    f\leftarrow x=\sum f_2(S(x))f_1=0. 
  \]
