...
\begin{definition}
  \index{Hopf module}
  Let $H$ be a Hopf algebra. A \emph{Hopf module}
  is a right $H$-module and a right $H$-comodule $M$
  such that 
  \begin{equation}
    \label{eq:HopfModule}
    \delta(m\leftarrow h)=(m_0\leftarrow h_1)\otimes m_1h_2
  \end{equation}
  for all $m\in M$ and $h\in H$. 
\end{definition}

If $M$ is a right $H$-module, then $M\otimes H$ is a right $H$-module with
\[
  (m\otimes x)\leftarrow h=(m\leftarrow h_1)\otimes xh_2
\]
for all $m\in M$ and $x,h\in H$. 
The compatibility
condition~\eqref{eq:HopfModule} means that the coaction $\delta\colon M\to
M\otimes H$ is an $H$-module homomorphism. 

\begin{example}
  Let $M=H$, $m\leftarrow h=mh$ and $\delta(m)=\Delta(m)$. 
  Then $M$ is an $H$-Hopf module. 
\end{example}

\begin{example}[trivial Hopf modules]
  \index{Hopf module!trivial}
  Let $V$ be a vector space and $M=V\otimes H$. Then $M$ 
  is an $H$-Hopf module with 
  \[
    (v\otimes h)\leftarrow h'=v\otimes hh',\quad 
    \delta(v\otimes h)=(v\otimes h_1)\otimes h_2
  \] 
  for $v\in V$ and $h,h'\in H$. 
\end{example}

The following example is important:

\begin{lemma}[Larson--Sweedler]
  \label{lem:LarsonSweedler}
  Let $H$ be a finite dimensional Hopf algebra with basis $h_1,\dots,h_n$.
  Let $f_1,\dots,f_n$ be its dual basis. 
  Then $H^*$ is a Hopf
  module with 
  \[
    f\leftarrow h=f_2(S(h))f_1,\quad 
  \delta(f)=\sum_{i=1}^n f(h_{i,2})f_i\otimes h_{i,1}
  \]
  for $f\in H^*$ and $h\in H$. 
\end{lemma}

\begin{proof}
  By Exercises...\framebox{?}, $H^*$ is a right $H$-module
  and $H$-comodule. Let us prove the compatibility condition. On the one 
  hand, 
  \[
  \delta(\varphi\leftarrow h) = f_2(S(h))f_1 = 
  \sum_{i=1}^n\varphi_2(S(h))f_i\otimes h_{i,1}\varphi_1(h_{i,2}).
  \]
  On the other hand, 
  \begin{align*}
    (\varphi_0\leftarrow h_1)\otimes \varphi_1h_2 
    &=\sum_{i=1}^n\varphi(h_{i,2})(f_i\leftarrow h_1)\otimes h_{i,1}h_2\\
    &=\sum_{i=1}^n\varphi(h_{i,2})(f_{i,2}(S(h_1))f_{i,1}\otimes h_{i,1}h_2.
  \end{align*}
  Evaluating the left tensor on $x\in H$, 
  \begin{align*}
    \sum
  \end{align*}
\end{proof}

\section{The fundamental theorem of Hopf modules}

The \emph{fundamental theorem of Hopf modules} states that every 
Hopf module of a finite-dimensional Hopf algebra 
is trivial.

\begin{theorem}[Larson--Sweedler]
  \label{thm:HopfModules}
  Let $H$ be a finite-dimensional Hopf algebra, 
  $M$ a Hopf module over $H$, and 
  \[
    V=M^{\operatorname{co}H}=\{m\in M:\delta(m)=m\otimes 1\}
  \]
  be the space of coinvariants of $M$. 
  Then $V\otimes H\to M$, $v\otimes h\mapsto (v\leftarrow h)$, is an
  isomorphism of $H$-Hopf modules. 
\end{theorem}

\begin{proof}
  We claim that $f\colon V\otimes H\to M$, $v\otimes h\mapsto (v\leftarrow h)$,
  is an isomorphism of Hopf modules. 

  First we prove that $f$ is $H$-linear:
  \begin{align*}
    &f(v\otimes h\leftarrow h') = f(v\otimes hh')=v\leftarrow (hh')
    =(v\leftarrow h)\leftarrow h'=f(v\otimes h)\leftarrow h
  \end{align*}
  for all $v\in V$ and $h,h'\in H$. 

  Now we prove that $f$ is $H$-colinear. 

  To construct the inverse of $f$ we proceed as follows. Let 
  $p\colon M\to V$, $m\mapsto (m_0\leftarrow S(m_1))$. The 
  map $p$ is well-defined...
  
  Let $g\colon M\to V\otimes H$, $m\mapsto p(m_0)\otimes m_1$. Wel claim
  that $g$ is the inverse of $f$. On the one hand, 
  \begin{align*}
    f(g(m)) &= 
  \end{align*}
  On the other hand, 
...
Let us see an easy application:

\begin{corollary}
  Let $H$ be a finite-dimensional Hopf algebra. If $I$ is a right ideal and a
  right coideal of $H$, then either $I=H$ or $I=\{0\}$. 
\end{corollary}

\begin{proof}
  Note that $I$ is a right $H$-module with
  right multiplication, that is $h\leftarrow h'=hh'$ for $h,h'\in H$. 
  Moreover, 
  $I$ is a right $H$-comodule with $\delta(h)=\Delta(h)$ for all $h\in H$. By Theorem~\ref{thm:HopfModules}, 
  $I\simeq I^{\operatorname{co}H}\otimes H$ 
  as $H$-Hopf modules. In particular, 
  \begin{equation}
    \label{eq:dim}
    \dim I=(\dim I^{\operatorname{co}H})(\dim H)\leq (\dim I)(\dim H).
  \end{equation}
  Thus $\dim I^{\operatorname{co}H}\in\{0,1\}$. If $\dim I^{\operatorname{co}H}=0$, 
  then~\eqref{eq:dim} implies that $\dim I=0$. Hence $I=\{0\}$. If 
  $\dim I^{\operatorname{co}H}=1$, then $I=H$. Indeed, 
  let $0\ne x\in I^{\operatorname{co}H}$. 
  Then 
  \[
    x=(\epsilon\otimes\id)\Delta(x)=(\epsilon\otimes\id)(x\otimes 1)=\epsilon(x)1.
  \]
  In particular, $\epsilon(x)\ne 0$. Hence $1\in I$ and threfore $I=H$. 
\end{proof}
...
\begin{definition}
  ...
  An element $t$ is called a \emph{left integral} if 
  $ht=\epsilon(h)t$ for all $h\in H$. 
\end{definition}

The set $\int_H^l$ of left integrals of $H$ is a subspace of $H$. 

Similarly, one defines a \emph{right integral}: it is
an element $t\in H$ such that 
$th=\epsilon(h)t$ for all $h\in H$. 
The set $\int_H^r$ of right integrals of $H$ is a subspace of $H$.  

\begin{example}
  Let $G$ be a finite group and 
  $H=KG$. Then $t=\sum_{g\in G}g\in\int_H^l\cap\int_H^r$. 
\end{example}

\begin{proposition}
  \label{pro:dual_op}
  Let $H$ be a finite-dimensional Hopf algebra. 
  If $H^*$ is an $H$-Hopf module with the structure of..., 
  then $\int_{H^*}^l = (H^*)^{\operatorname{co}H}$. 
\end{proposition}

\begin{proof}
  Let $h_1,\dots,h_n$ be a basis of $H$ and $f_1,\dots,f_n$ be its
  dual basis. 
  An element $\varphi\in H^*$ is a left integral of $H^*$ 
  if and only if $\varphi(h_2)h_1=\varphi(h)1_H$ for all $h\in H$. Since 
  \[
  \varphi(h_2)h_1=\varphi(h)1_H\Longleftrightarrow 
  \sum_{i=1}^n\varphi(h_{i,2})f_i\otimes h_{i,1}=\varphi\otimes 1_H
  \Longleftrightarrow \delta(\varphi)=\varphi\otimes 1_H, 
  \]
  the claim follows. 
\end{proof}
...
\begin{theorem}
  Let $H$ be a finite-dimensional Hopf algebra. 
  \begin{enumerate}
    \item Both $\int_H^l$ and $\int_H^r$ are one-dimensional. 
    \item $S(\int_H^l)\subseteq\int_H^r$. 
  \end{enumerate}
\end{theorem}

\begin{proof}
  Let us prove the first claim. By Theorem~\ref{thm:HopfModules}, 
  $H^*\simeq (H^*)^{\operatorname{co}H}\otimes H$ as Hopf modules. Then 
  \[
    \dim H=\dim H^*=(\dim (H^*)^{\operatorname{co}H})(\dim H).
  \]
  and hence $\dim (H^*)^{\operatorname{co}H}=1$. Now 
  the claim follows from Proposition~\ref{pro:dual_op}. 
...
% for this I need integrals

\begin{corollary}
  Let $H$ be a finite-dimensional Hopf algebra. Then $S$ is bijective. 
\end{corollary}

\begin{proof}
  Recall that $S\colon H\to H$ is a linear map. Let $x\in\ker S$. Since 
  $H^*$ is a Hopf $H$-module (see Lemma~\ref{lem:LarsonSweedler}), 
  $S(x)=0$ implies that
  \[
    f\leftarrow x=f_2(S(x))f_1=0. 
  \]
  By Theorem~\ref{thm:HopfModules}, $H\simeq...$. 
  By Lemma... $(H^*)^{\operatorname{co}H}=\int_{H}^l$. 
\end{proof}

\section{Frobenius algebras}

\begin{definition}
  Let $A$ be a $K$-algebra. We say that $A$ is \emph{Frobenius} if there exists
  a non-degenerate bilinear form $\kappa\colon A\times A\to K$ such that
  $\kappa(ab,c)=\kappa(a,bc)$ for all $a,b,c\in A$. 
\end{definition}
...
\begin{theorem}
  Let $H$ be a finite-dimensional Hopf algebra. Then $H$ is
  a Frobenius algebra. 
\end{theorem}
...
Theorem~\ref{thm:Maschke} was in fact proved by... Why is then called
Maschke's theorem? 
...
\begin{align*}
h_{2}a_{i}\otimes h_{1}b_{i} & =a_{i}h_{1}\otimes b_{i}h_{2},\\
a_{i,1}\otimes a_{i,2}\otimes b_{i} & =a_{i}\otimes a_{j}\otimes b_{i}b_{j},\\
a_{i}\otimes b_{i,1}\otimes b_{i,2} & =a_{i}a_{j}\otimes b_{j}\otimes b_{i}.
\end{align*}
for all $h\in H$. 
\end{exercise} 
...
\begin{exercise}
Let $H$ be a quasitriangular Hopf algebra with $R=a_{i}\otimes b_{i}$.
...
Let $H$ be a Hopf algebra. A \emph{Yetter--Drinfeld module} over $H$ is a
...
h_{1}v_{-1}\otimes(h_{2}\rightarrow v_{0})
=(h_{1}\rightarrow v)_{-1}h_{2}\otimes(h_{1}\rightarrow v)_{0}.
\end{equation}
\end{exercise}
...
Yetter--Drinfeld module over $H$, where 
\begin{align*}
h\rightarrow(v\otimes w) & =(h_{1}\rightarrow v)\otimes(h_{2}\rightarrow w),\\
...
c_{V,W}^{-1}c_{V,W}(v\otimes w) & =c_{V,W}^{-1}((v_{-1}\to w)\otimes v_{0})\\
 ...
 & =v_{0}\otimes(\epsilon(v_{-1})1\to w)\\
 & =v\otimes w,
\end{align*}
